/obj/item/clothing/gloves/fallout/
    name = "generic fallout gloves"
    desc = "this doesn't even exist"
    icon = 'fallout/icons/objects/clothing/gloves.dmi'
    worn_icon = 'fallout/icons/mob/clothing/hands.dmi'

/obj/item/clothing/gloves/fallout/bracers
    name = "legionary bracers"
    desc = "Bracers. Often worn by Legionaries to protect their forearms against rudimentary attacks."
    icon_state = "leg_bracers"
    armor = list(melee = 10, bullet = 5, laser = 5, energy = 5, bomb = 0, bio = 0, rad = 0, fire = 5, acid = 0)

/obj/item/clothing/gloves/fallout/darkwraps
    name = "dark handwraps"
    desc = "Some dark handwraps. Good for getting extra grip or protecting from minor cuts."
    icon_state = "leg_darkwraps"

/obj/item/clothing/gloves/fallout/tanwraps
    name = "tan handwraps"
    desc = "Some tan handwraps. Good for getting extra grip or protecting from minor cuts."
    icon_state = "leg_tanwraps"
