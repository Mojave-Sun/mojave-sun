...
///Returns a list of humans via get_dist and same z level method, very cheap compared to range() unless there's so many humans or range() is being used with a very small area
/proc/cheap_get_humans_near(atom/movable/source, distance)
	. = list()
	for(var/human in GLOB.human_list)
