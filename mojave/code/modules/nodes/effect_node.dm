...
/obj/effect/ai_node //A effect that has a ai_node datum in it, used by AIs to pathfind over long distances as well as knowing what's happening at it
	...
	icon_state = "x5" //Pure white 'X' with blacken'd borders
	...
	var/list/weights = list(
		IDENTIFIER_HOSTILE_MOB = list(NODE_LAST_VISITED = 0),
		)
 ...
 * this node was visited by a particular node-using thing
 * Parameter call example
 * GetBestAdjNode(list(NODE_LAST_VISITED = -1), IDENTIFIER_HOSTILE_MOB)
...
/obj/effect/ai_node/proc/make_adjacents()
	adjacent_nodes = list()
	for(var/atom/node in GLOB.all_nodes)
		if(node == src)
			continue
